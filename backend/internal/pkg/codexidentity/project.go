...
func ProjectHeaders(headers http.Header, plan OutboundPlan) (http.Header, error) {
	if err := plan.Validate(); err != nil {
		return nil, err
	}
	nextHeaders := make(http.Header, len(headers))
	...
	for _, patch := range plan.Patches {
		if patch.Carrier == CarrierHeader {
			nextHeaders.Set(patch.Path, patch.Value)
			continue
		}
		if patch.Carrier != CarrierTurnMetadata || !strings.HasPrefix(patch.Path, headerTurnMetadataPrefix) {
			continue
		}
		if err := patchHeaderTurnMetadata(nextHeaders, strings.TrimPrefix(patch.Path, headerTurnMetadataPrefix), patch.Value); err != nil {
			return nil, err
		}
	}
	return nextHeaders, nil
}

func ProjectBody(body []byte, plan OutboundPlan) ([]byte, error) {
	if err := plan.Validate(); err != nil {
		return nil, err
	}
	nextBody := append([]byte(nil), body...)
	for _, patch := range plan.Patches {
		var err error
		switch {
		case patch.Carrier == CarrierBody:
			nextBody, err = sjson.SetBytes(nextBody, patch.Path, patch.Value)
		case patch.Carrier == CarrierTurnMetadata && strings.HasPrefix(patch.Path, bodyTurnMetadataPrefix):
			nextBody, err = patchBodyTurnMetadata(nextBody, strings.TrimPrefix(patch.Path, bodyTurnMetadataPrefix), patch.Value)
		default:
			continue
		}
