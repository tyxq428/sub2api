...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.UsesOpenAICodexProtocol() && account.Platform != PlatformGrok {
		...
		ensureCodexIdentityHeadersForAccount(upstreamReq.Header, account)
		enforceCodexIdentityHeadersForAccount(upstreamReq.Header, account, "")
...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	missingTerminalErr := func() (*OpenAIForwardResult, error) {
		...
		if !clientOutputStarted {
			return result, markOpenAIAttemptMaybeSent(s.newOpenAIStreamFailoverError(c, account, false, requestID, nil, message))
		}
