...
class VerifierTests(unittest.TestCase):
    def setUp(self):
        ...
        gates=['zero_errors','no_upstream_duplicates','valid_upstream_inputs','upstream_count_exact','duration_complete','sample_count_complete','p95_within_10pct','stable_rss_within_20pct','fake_tracker_bounded']
        self.result={'parameters':{'seconds':7200,'warmup':300,'rps':20,'baseline':BASELINE,'candidate':CANDIDATE},'network_internal':True,'real_model_requests':0,'actual_elapsed_s':7500.1,'passed':True,'gates':dict.fromkeys(gates,True),'run_id':self.run_id,'upstream':{'requests':300020,'duplicates':0,'invalid':0,'tracker':{'bytes':65536},'memory':{'heapUsed':8000000}},'cleanup':[{'name':n,'removed':True} for n in names],'cleanup_complete':True,'variants':{}}
    ...
    def test_unbounded_fake_tracker_rejected(self):
        self.result['upstream']['tracker']['bytes']=3*1024*1024
        with self.assertRaisesRegex(VerificationError,'tracker memory'): self.check()
    def test_fake_heap_regression_rejected(self):
        self.result['upstream']['memory']['heapUsed']=256*1024*1024
        with self.assertRaisesRegex(VerificationError,'V8 heap'): self.check()
